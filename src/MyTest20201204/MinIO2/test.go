...
func main() {
	...
	/*str := `{
		...
		n, err := minioClient.PutObject(ctx, bucketName, "myobject2.json", reader, -1, minio.PutObjectOptions{ContentType: "application/octet-stream"})
		if err != nil {
			fmt.Println(err)
			return
		}
	...
	// 根据名称获取内容
	object, err := minioClient.GetObject(ctx, bucketName, "tet00Xl33ejxUy8v6_0.json", minio.GetObjectOptions{})
	if err != nil {
		fmt.Println(err)
		return
	}
	localFile, err := os.Create("./local-file22444.json")
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	// 	查询元数据
	objInfo, err := minioClient.StatObject(ctx, bucketName, "tet00Xl33ejxUy8v6_06.json", minio.StatObjectOptions{})
	if err != nil {
		fmt.Println("err:", err)
