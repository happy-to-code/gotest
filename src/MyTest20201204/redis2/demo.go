...
// 向Redis中set值
func SetValue(client *redis.Client, key string, value interface{}, expiration int) {
...
// 从Redis中获取值
func GetValue(client *redis.Client, key string) (error, string) {
...
// 根据key从Redis中删除某个值
func DelValue(client *redis.Client, key string) error {
...
// 从Redis中获取单个值
func MGetValue(client *redis.Client, keys []string) (error, map[string]string) {
	...
	// 返回体
	var bkMap = make(map[string]string)
	for i, k := range keys {
		vObject := val[i]
		if vObject != nil {
			bkMap[k] = vObject.(string)
		}
	}
...
func main() {
	defer client.Close()
	// SetValue(client, "jkjkjk", "jkjkjk", 0)

	err, s := GetValue(client, "jkjkj2k")
	if err != nil {
		fmt.Printf("err:%+v\n", err)
		panic(err)
	}
	fmt.Printf("%+v\n", s)
	// SetValue(client, "Keyyy99", "vlll", 0)
	//
	// _, vs := MGetValue(client, []string{"k","fd", "kk", "ss"})
	// fmt.Printf("%+v\n", vs)

	// err := DelValue(client, "LLLL")
	// fmt.Printf("err:%+v\n", err)
